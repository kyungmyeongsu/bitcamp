...
public class C2 extends C{
    ...
    //    C2에서 접근할 수 없다. 접근할 수없으니, 오버라이딩은 불가능하다!
    /*
    @Override
    private void m1() {}
    */
    
    // 어? 강사님, @Override 빼니깐 m1() 정의할수 있는데요?
    // => 이건 오버라이딩이 아니라 C2에 새로 메서드가 추가 된 것이다.
    // => 오버라이딩이라면 @Override 애노테이션을 붙였을 때 오류가 나지 말아야한다!
    private void m1() {}
    ...
    */
    ...
    // 오버라이딩 할때 (메서드를 재정의 할 때)
    // 원본보다 접근 범위를 확대할 순 있지만 좁힐 수는 없다.
    //@Override void m2() {} // 접근 범위를 protected에서 (default)로 좁혔기 때문에
                             // 컴파일 오류!
    
    // => 접근 범위를 확대하는 것은 괜찮다.
    //@Override public void m2() {} // protected 보다 접근 범위를 확대 했기 때문에OK!
