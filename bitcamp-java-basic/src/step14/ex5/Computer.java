...
public interface Computer {
    ...
    // 문제점
    // => 규칙을 변경하면, 그 규칙에 따라 만든 모든 클래스를 변경해야한다.
    // => 내가만든 큐칙에 따라 내가 클래스를 만들었다면 내가 변경하면 되지만,
    //    내가 만든 규칙을 다른 많은 개발자들이 가져가서 클래스를 만들어 사용하고 있다면,
    //    내가 쉽게 규칙을 변경할 상황이 아닌것이다.
    //    내가 규칙 즉 인터페이스의 메서드를 변경하거나 제거, 추가 하는 순간
    ...
    // 바로 다음 문법을 사용하라!
    // "디폴트 메서드(default method)" - JDK8에 추가된 문법이다.
    default void touch() {
        //구현할 코드있으면 작성하고 없으면 빈채로 내버려둔다.
    }
    
    // 위에touch()라는 새 규칙을 추가하더라도
    // 기존에 작성한 FirstComputer, SecondComputer, ThirdComputer 클래스 모두
    // 컴파일 오류가 발생하지 않는다.
    // 왜? 구현된 메서드이기 때문이다.
    // ㅇ\
    // 가능한 일반 클래스의 메서드 처럼 사용하지 말아라!
    // 새 규칙을 추가하는 의미오 default 메서드를 사용해야지
    // 일반 클래스
}
