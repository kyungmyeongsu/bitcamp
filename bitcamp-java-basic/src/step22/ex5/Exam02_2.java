...
// 1바이트를 여러번 읽을 경우 매번 바이트의 위치를 찾아야 하기 떄문에
// 평균 탐색시간이 누적되어 한 번에 1000 바이트를 읽는 것 보다 시간이 오래 걸리다.
//
// 그러면 RAM에 배열의 크기를 크게 잡으면 좋지 않겠는가?
// => PC 처럼 소수의 프로그램이 동시에 실행될 떄는 상관없지만,
//    서버처럼 데이터를 읽는 코드가 동시에 수천개에서 수십만개일 떄는
//    아무리 작은 크기의 메모리라도 크기를 유지하고 있다.
// => 물론 프로그램의 용도나 목적에 따라 버처의 크기가 