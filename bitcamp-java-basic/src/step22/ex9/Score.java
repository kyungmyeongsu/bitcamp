...
public class Score implements Serializable {
    ...
    // 인스턴스 변수의 구분
    // => name, kor, eng, math 는 사용자다 입력한 값을 저장하는 변수이다.
    String name;
    int kor;
    int eng;
    int math;
    ...
    // sirialize 대상에서 제외해야한다.
    // 왜? 값의 왜곡을 막기 위함이다.
    //
    // 그럼 sum 과 aver는 어떻게 처리해야하는가?
    // => serialize 데이터를 읽는 쪽에서 데이터를 다 읽은 후 계산을 수행해서 값을 처리해야한다.
    //    그래야만 데이터 왜곡을 막을 수 있다.
    //
    // 데이터 왜곡? kor, eng, math 에 저장된 값과 sum과 aver에 저장된 값이 일치하지 않는 경우를 말한다.
    // 이런 변수는 seialize 대상에서 제외해야 하는데,
