...
public class Exam02_1 {
    ...
    //    => JVM에서 오류가 발생한 경우에는 계속 실행해봐야 소용없다.
    //       근본적으로 문제를 해결할수 없다.
    //    => 오류의 예:
    //       스택 오버플로우 오류, VM관련 오류, AWT 원도우 관련 오류, 스레드 종료 오류 등
    // 2) Exception
    //    => 애플리케이션에서 발생시킨 오류이다.
    //    => 개발자가 사용하는 클래스이다.
    //    => 적정한 조치를 취한후 계속 시스템을 실행하게 만들 수있다.
    //    => 오류의 예:
    //       배열의 인덱스가 무효한 오류, I/O 오류, SQL오류, Parse 오류, 데이터 포멧 오류 등
    //
    
    // 오류를 던진다면 반드시 메서드 선언부에 어떤 오류를 던지는지 선언해야한다.
    static void m() throws Throwable {
        throw new Throwable();
    }
    
    static void m2() throws Error {
        throw new Error(); // OK!하지만 이계열의 클래스를 사용하지 말라!
