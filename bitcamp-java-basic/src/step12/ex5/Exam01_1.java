...
//    => Set은 집합을 의미한다.
//    => 집합에서는 중복값을 허용하지 않는다.
//    => 값의 중복여부는 hashCode()의 리턴 값이 같고, equals()의 검사 결과가 true일 때
//       같은 값으로 취급한다.
//    => 즉 중복된 값을 저장하고 싶지 않을 떄 HashSet을 사용한다.
//    => 값을 저장할 때 순서대로 저장하지 않는다.
// 2) 값을 순서대로 저장하지 않는다.
...
public class Exam01_1 {
    public static void main(String[] args) {
        ...
        set.add(v4);
        set.add(v5); 
        // v5와 v3는 다른 인스턴스 이다. 하지만 두 객체에 들어있는 문자열은 같다.
        // String 클래스는 인스턴스가 다르더라도 값이 같으면,
        // hashCode()의 리턴 값이 같도록 오버라이딩 하였다.
        // 또한 equals()의 리턴 값도 true 오버라이딩 하였다.
        
        // HashSet은 값의 중복을 검사할떄 인스턴스의 주소를 비교하지 않고
        // hashCode()의 리턴 값과 equals()의 리턴 값으로 판단한다.
        // 따라서 v5는 비록 v3와 다른 인스턴스이지만,
        // hashCode()의 리턴 값이 같고 두 객체에 대해  equals()호출하면
        //
