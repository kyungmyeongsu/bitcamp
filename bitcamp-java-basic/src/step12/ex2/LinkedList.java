...
public class LinkedList {
    ...
    // 일반적으로 클래스인 패키지 멤버 클래스로 정의 하지 않고
    // LinkedList 안에 정의한다.
    // 이렇게 클래스 안에 정의괸 클래스를 "중첩 클래스(nested class)" 또는
    ...
    class Bucket {
        ...
        Bucket prev; //이변수에는 이전 객체 주소를 저장한다.
    }
    
    
    Bucket head; // 리스트에서 맨 앞 객차의 주소를 저장하는 인스턴스 변수
    Bucket tail; // 리스트에서 맨 뒤 객차의 주소를 저장하는 인스턴스 변수
    ...
    public void add(int i, Object value) {
        ...
        while (cursor != tail) {
            if (count == i) {
                ...
                } else { // 이전에 다른 객차가 없다면 이 객차를 해드로 만든다.
                    head = bucket;
                }
