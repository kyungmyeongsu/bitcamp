...
public class AppConfig {
    ...
    // 스프링 IoC 컨테이너 에서 자동 생성하지 않는 객체는 
    // 팩토리 메서드를 만들어 그 객체를 리턴하도록한다.
    // 단, 스프링 IoC 컨테이너에게 이 메서드를 호출하여
    // 이 메서드가 리턴한 객체를 컨테이너에 보관하도록 명령을 내려야하는데
    // 메서드 선언 앞에 @Bean 애노테이션을 붙이면된다.
    @Bean // 스프링 IoC 커테이너에게 이메서드 를 호출하라는 명령이다.
          // 스프링 IoC 컨테이너는 이메서드를 호출한 후 그 리턴 값을 컨테이너에 보관한다.
