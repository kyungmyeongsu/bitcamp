...
// 하는 일:
// => 클라이언트가 요청이 들어오면 그 요청을 컨트롤러에게 위임하는 역할을 한다.
// => 스프링 IoC 컨테이너를 사용하는 컨트롤러 및 DAO 등을 관리한다.
// 
...
public class DispatcherServlet implements Servlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        // 최초의 요청이 들어오면 딱 한번 실행한다.
        // 그래서 서블릿이 실행하는데 필요한 자원을 준비하는 코드를 여기 두면 된다.
        this.config = config;
        
        // 스프링 IoC컨테이너 객체 생성
        // => 파라미터로 넘기는 값은 스프링 IoC컨테이너 관련 
        iocContainer = new AnnotationConfigApplicationContext(AppConfig.class);
    }

    @Override
    public ServletConfig getServletConfig() {
        // 서블릿 설정 정보를 리턴하는 일을한다.
    ...
    @Override
    public void destroy() {
        // 서버를 종료하기전에 서블릿 컨테이너가 호출한다.
        // => 여기에는 서블릿이 실행되는 동안 만들었던 자원을 해제하는 코드를 두면 된다.
        //    예를 들어, 파일이나 DB커넥션, 소켓 등의 자원을 해제한다.
