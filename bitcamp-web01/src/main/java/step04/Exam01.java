...
Accept-Language: ko-KR,ko;q=0.9,en-US;q=0.8,en;q=0.7,la;q=0.6
(빈 줄)  <== empty line 
name=AB%EA%B0%80%EA%B0%81&age=30    <== post 요청은 빈 줄 다음에 데이터를 보낸다.  
*/

// GET vs. POST
// [GET]
//    - URL에 데이터가 포함되기 때문에 바이너리 데이터를 보낼수 없다.
//      바이너리 데이터를 BASE64 형식(텍스트형식)으로 변환하여 보낼 수는 있다.
//    - 대용량의 데이터를 보낼수 없다.
//      대부분의 웹서버는 request-line 및 header의 최대 크기가 제한되어있다.
//      => 그래서 파일 업로드나 게시글 등록, 변경에서는 GET을 요청하지 않는다.
//    - 보내는 데이터가 타인에게 노출된다. 보안에 취약
//      URL에 데이터가 포함된다.
//      => 그래서 로그인 값을 GET요청으로 보내지 않는다.
//    - URL에 값을 동시에 보낼 때 사용한다.
//      왜? URL에 값이 표함되기 때문이다.
//      => 예) 검색을 요청하는 URL
//    - 즐겨찾기에 저장할 수 있으며, 방문내역에 자동으로 저장될수있다.
//      => 그래서 더더욱 로그인에는 GET을 사용하면 안된다.
//      => 왜? 즐겨찾기나 방문기록에 URL정보가 자동으로 저장되기 때문이다.
// [POST]
//    - 멀티파트 형식으로 바이너리 데이터를 보낼수 있다.
//    - 대용량의 데이터를 보낼 수있다.
//      빈 줄 다음에 데이터를 보내기 때무에 데이터의 크기에 제한이 없다.
//      서버 쪽에서 허용하는 만큼 보낼수 있다.
//      => 파일업로드나 게시글 등록, 변경에 주로 사용한다.
//    - 보내는 데이터가 웹브라우저의 URL 주소창에 노출되지 않는다.
//      => 그래서 로그인 값을 보낼 깨 이방식으로 사용한다.
//    - 즐겨찾기에 저장해봐야 소용없다. 방문기록에 저장된 것은 무용하다.
//      => 왜? URL만 저장되기 때문이다. 즉 데이터가 포함되어 있지 않기 떄문이다.
//      => 게시물 조회, 검색 결과 조회와 같은 데이터 조회 결과를 출력하는 URL 방식은


// BASE64
// - 바이너리 데이터를 텍스트로 변환하여 전송할 때 사용한다.
// - 바이너리 데이테를 ASCII 코드의 문자로 변환하는 방식이다.
// - 이렇게 텍스트로 만든 데이터를 OS에 상관없이, 프로그램밍 언어에 상관없이,
//   네트워크 프로토콜에 상관없이 언제든지 주고 받을 수 있다는 장점이있다.
