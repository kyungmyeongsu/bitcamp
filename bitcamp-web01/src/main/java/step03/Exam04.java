...
public class Exam04 extends GenericServlet {
    ...
    public void service(
        ...
        //      2) URL 인코딩: "AB%EA%B0%80%EA%B0%81" (41 42 25 45 41 25 42 30 25 38 30 25 45 41 25 42 30 25 38 31)
        //    톰켓서버:
        //      3) URL 디코딩을 수행하여 본래의 UTF-8로 변환: "AB가각" (41 42 ea b0 80 ea b0 81)
        //         - UTF-8은 자바 내부에서 사용하지 않는다.
        //         - 자바의 String 객체는 UTF-16을 사용한다.
        //      4) String 객체(UTF-16): "ABêë" (0041 0042 00ea 00b0 0080 00ea 00b0 0081)
        //         - 각 바이트를 UTF-16으로 변환한다.
        //         - 클라인트가 보낸 데이터가 어떤 문자표에 따라 만든 데이터인지 알려주지 않으면,
        //           모조건 ISO-8859-1(ISO-latin-1) 이라고 가정하고 무조건 0x00 1바이트를 붙인다.
        //         - 영어인 경우에는 앞에 0x00을 붙이면 UTF-16이기 때문에 아무런 문제가 없다.
        //         - 그러나 한글일 경우에는 UTF-8 각 바이트에 그냥 0x00을 붙이면
        //           올바르지 않은 UTF-16코드가 되기 때문에 출력할 때 이상한 글자가 보이는 것이다.
        // 해결책?
        // 1) POST 명령으로 보낸 데이터인 경우
        // => 최초로 값을 꺼내기 전에(getParameter()를 최초로 호출하기 전)
        //    클라이언트가 보낸 문자가 어떤 문자코드로 되어있는지 알려둬야한다.
        //          request.setCharacterEncoding("UTF-8");
        // => 즉 클라이언트로 부터 받은 코드를 다음과 같이 바꾼다.
        //    UTF-8: "AB가각" (41 42 ea b0 80 ea b0 81)
        //    String 객체(UTF-16): "AB가각" (0041 0042 AC00 AC01)
        // 2) GET 명령으로 보낸 데이터인 경우
        ...
        // 단, tomcat 8 이상 부터는 server.xml에 위와 같이 편집할 필요가 없다.
        // GET요청으로 보낸 UTF-8 데이터는 UTF-16으로 잘 변환된다.
        //
