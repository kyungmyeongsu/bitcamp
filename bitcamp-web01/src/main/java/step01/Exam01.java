...
// => javax.servlet.Servlet 규칙에 따라 클래스를 만들어야한다.
// => Servlet 인터페이스
//    호출자? 즉 이 규칙에 따라 호출하는 자는 누구냐?
//    - 서블릿 컨테이너(servlet container)이다.
//      예) 톰켓 서버
// => 일반적으로 실행할 수있는 자바 클래스를 만들 듯이 main() 메서드를 만들어봐야 소용없다.
// => 서블릿 컨테이너가 호출할 클래스이기 때문에,
//    서블릿 컨테이너가 호출하는 메서드를 갖는 클래스를 만들어야 한다.
// => 서블릿 컨테이너가 어떤 메서드를 호출할지 이미 규칙이 정의되어 있다.
//    바로 그 규칙이 Servlet 인터페이스 인 것이다.
//
// 이렇게 만든 서블릿을 실행 하려면 서블릿 컨테이너에 등록해야힌다.
// @WebServlet("실행할 때 사용할 이름")
// => 실행할 때 사용할 이름
//    - root 경로(/)로 시작한다.
//    - 웹브라우저에서 요청할 때 사용할 이름이다.
//    - 파일 경로 처럼 중간에 슬래시(/)를 사용하여 이름을 길게 지을수 있다.
...
public class Exam01 implements Servlet {
    ...
    @Override
    public void destroy() {
        // 웹 애플리케이션을 멈추거나 서블릿 컨테이너를 종료할 떄 호출된다.
    ...
    public String getServletInfo() {
        // 서버 관리자 메뉴에서 서블릿에 대한 간단란 정보를 출력하려 할때 호출된다.
