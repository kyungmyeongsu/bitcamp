...
- 현재는 java 언어만 가능하다.
- JSP를 만들 당시에는 JSP 페이지를 작성할 때 Java 언어 외에 다른언어를 사용할 것을 대비해서 
  language라는 속성을 만들었다.
- 초창기에는 JSP 페이지에 다른언어를 삽입하려는 시도가 있었다.
  지금은 Java 만 넣는다.
...
- JSP 엔인이 JSP 파일을 읽을 때 이 속성에 지정된 문자표를 사용하여 읽는다.
- 생략해도된다.
...
- 값을 true로 설정하면 지시문 사이에 있는 공백(white space; 스페이스, 탭, 줄바꿈)을 제거한다.

[buffer 속성]
- 출력 버퍼의 크기를 지정한다.
- 기본 값은 "8kb"이다. 즉 8196 바이트를 초과하면 autoFlush 정책에 따라 처리 된다.

[autoFlush 속성]
- 기본 값은 true이다. 즉 버퍼가 꽉 차면 바로 클라이언트로 출력한다.
- false로 설정 되어 있다면, 버퍼가 꽉 찼을 때 오류가 발생 한다.
</pre>
