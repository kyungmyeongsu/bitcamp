...
/* natural join 의 문제점 
 * 1) 두 테이블의 조인 기준이 되는 컬럼 이름이 다를 때 연결 되지 못한다.
...
/* [inner] join ~ on 의 문제점
   => 반드시 on 에서 지정한 컬럼의 값이 같을 경우에만 
        두 테이블의 데이터가 연결된다.
   => 같은 값을 갖는 데이터가 없다면 연결된지 않고, 결과로 출력되지 않는다.
*/
/* 전체 강의 목록 */
 ...
 * 강의실이 아직 지정되지 않은 강의의 경우 강의실 테이블의 데이터와 연결 하지 못해
 * 결과로 출력되지 않는 문제가 있다. */

/* => 만약 기준 컬럼의 값과 일치하는 데이터가 없어서
 *    다른 테이블의 데이터와 연결되지 않았다 하더라도
 *    결과로 뽑아내고 싶다면 outer join을 사용하라! */
/* 즉 아직 강의실을 배정 되지 않는 강의 데이터도 출력하고 싶을 경우
 * 출력하고 싶은  */
select lno, 
       l.titl, 
       r.rno, 
